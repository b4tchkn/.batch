"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const vscode = require("vscode");
const clipboard = vscode.env.clipboard;
function getContent() {
    return clipboard.readText();
}
exports.getContent = getContent;
function setContent(content) {
    return clipboard.writeText(content);
}
exports.setContent = setContent;
function vscodeCopy() {
    return __awaiter(this, void 0, void 0, function* () {
        yield vscode.commands.executeCommand("editor.action.clipboardCopyAction");
    });
}
exports.vscodeCopy = vscodeCopy;
function vscodeCut() {
    return __awaiter(this, void 0, void 0, function* () {
        yield vscode.commands.executeCommand("editor.action.clipboardCutAction");
    });
}
exports.vscodeCut = vscodeCut;
function vscodePaste() {
    return __awaiter(this, void 0, void 0, function* () {
        yield vscode.commands.executeCommand("editor.action.clipboardPasteAction");
    });
}
exports.vscodePaste = vscodePaste;
//# sourceMappingURL=utils.js.map