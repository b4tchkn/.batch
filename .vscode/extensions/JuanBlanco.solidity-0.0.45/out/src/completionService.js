...
class CompletionService {
    ...
    getAllCompletionItems(documentText, documentPath, packageDefaultDependenciesDirectory, packageDefaultDependenciesContractsDirectory) {
        if (this.rootPath !== 'undefined' && this.rootPath !== null) {
            ...
            contracts.addContractAndResolveImports(documentPath, documentText, projectService.initialiseProject(this.rootPath, packageDefaultDependenciesDirectory, packageDefaultDependenciesContractsDirectory));
            let completionItems = [];
...
function CreateCompletionItem(label, kind, detail) {
    let completionItem = vscode_languageserver_1.CompletionItem.create(label);
